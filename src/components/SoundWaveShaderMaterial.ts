...
const fragmentShader = `
  ...
  // Check if a ray segment from 'from' to 'to' is blocked by any obstacle
  bool isPathBlocked(vec3 from, vec3 to) {
    ...
    for (int i = 0; i < ${MAX_OBSTACLES}; i++) {
      ...
      // Transform ray into obstacle local space
      ...
      if (u_obstacleShapes[i] == 0) {
        ...
        // Hit if the slab overlap is valid and falls within the segment [0, segLen]
        if (tNear < tFar && tFar > 0.05 && tNear < segLen - 0.05) {
          return true;
        }
      } else {
        // Ray-Cylinder intersection (Y-axis aligned, radius = halfSize.x, half-height = halfSize.y)
        float r = halfSize.x;
  ...
  vec3 heatmap(float v) {
      ...
      // Branchless colormap interpolation
  ...
  void main() {
    // If this point is inside an obstacle, render it as a solid dark surface
    ...
    for (int i = 0; i < ${MAX_SOURCES}; i++) {
      ...
      // Check line-of-sight: skip this source if an obstacle blocks the path
      if (u_numObstacles > 0 && isPathBlocked(pos, vWorldPosition)) continue;
